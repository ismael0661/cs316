...
joinWith y [] = []
joinWith y (x:[]) = x 
joinWith y (x:xs) = x ++ [y] ++ joinWith y xs
...
eqFindInTree a (Node l x r)
                        | a == x = True
                        | a /= x = (eqFindInTree a l) || (eqFindInTree a r)
...
moveLeft (AtEnd []) = AtEnd []
moveLeft (AtEnd (a:before)) = Within before a []
moveLeft (Within [] point after) = Within [] point after
moveLeft (Within (a:before) point after) = Within before a (point:after)
...
backspace (AtEnd before) = AtEnd (init before)
backspace (Within before point after) = Within (init before) point after
...
searchRightFor x (Within before point after) 
                | x /= point = searchRightFor x (moveRight(Within before point after))
                | x == point = Within before point after
searchRightFor x (AtEnd (a:before)) = searchRightFor x (Within [] a before) 
...
execute ((Insert x):xs) (cursor) = execute (xs) (insert x (cursor))
execute (Backspace:xs) (cursor) = execute (xs) (backspace(cursor))
execute ((SearchRightFor x):xs) (cursor) = execute (xs) (searchRightFor x (cursor))
...
expects (b:bs) = Input (Output (b==True) (expects bs)) (Output (b==False) (expects bs))
expects [] = End
